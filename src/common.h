...
#pragma warning(disable                                                        \
                : 4244 4018 4100 4610 4510 4127 4512 4146 4267 4503 4800 4706)
#endif
...
inline bool atomicCompareAndExchange(volatile uint32_t *v, uint32_t newValue,
                                     uint32_t oldValue) {
#if defined(_WIN32)
...
#if defined(_MSC_VER)
  return _InterlockedExchangeAdd(reinterpret_cast<volatile long *>(dst),
                                 delta) +
         delta;
#else
...
inline float atomicAdd(volatile float *dst, float delta) {
  ...
  do {
    ...
    newVal.f = oldVal.f + delta;
      ...
      !atomicCompareAndExchange((volatile uint32_t *)dst, newVal.i, oldVal.i));
  return newVal.f;
}
...
inline Float signum(Float value) { return std::copysign((Float)1, value); }
...
using ordered_lock = std::shared_timed_mutex;
...
inline std::vector<std::string> &str_tokenize(const std::string &s, char delim,
...
inline void jet(VectorXf &X, MatrixXu8 &C, float min, float max) {
  for (int i = 0; i < X.size(); ++i) {
    ...
    C.col(i) << (uint8_t)(r * 255.f), (uint8_t)(g * 255.f),
        (uint8_t)(b * 255.f), (uint8_t)255;
  }
}
