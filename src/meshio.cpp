...
void write_mesh(const std::string &filename, const MatrixXu &F,
  ...
  if (extension == ".ply")
    ...
    throw std::runtime_error("write_mesh: Unknown file extension \"" +
                             extension + "\" (.ply/.obj are supported)");
}
...
void load_ply(const std::string &filename, MatrixXu &F, MatrixXf &V,
  ...
  if (vertexCount == 0 && faceCount == 0)
    throw std::runtime_error("PLY file \"" + filename +
                             "\" is invalid! No face/vertex/elements found!");
  else if (!pointcloud && faceCount == 0)
    throw std::runtime_error("PLY file \"" + filename +
                             "\" is invalid! No faces found!");
  ...
  auto rply_vertex_normal_cb = [](p_ply_argument argument) -> int {
    ...
    if (data->progress && coord == 0 && index % 500000 == 0)
      data->progress("Loading vertex normal data",
                     index / (Float)data->N.cols());
    return 1;
  };
  ...
  if (!ply_set_read_cb(ply, "vertex", "x", rply_vertex_cb, &vcbData, 0) ||
    ...
    ply_close(ply);
    throw std::runtime_error("PLY file \"" + filename +
                             "\" does not contain vertex position data!");
  }

  if (pointcloud && faceCount == 0) {
    N.resize(3, vertexCount);
    if (!ply_set_read_cb(ply, "vertex", "nx", rply_vertex_normal_cb, &vncbData,
                         0) ||
        !ply_set_read_cb(ply, "vertex", "ny", rply_vertex_normal_cb, &vncbData,
                         1) ||
        !ply_set_read_cb(ply, "vertex", "nz", rply_vertex_normal_cb, &vncbData,
                         2)) {
      ply_close(ply);
      throw std::runtime_error(
          "PLY file \"" + filename +
          "\" does not contain vertex normal or face data!");
    }
  } else {
    if (!ply_set_read_cb(ply, "face", "vertex_indices", rply_index_cb, &fcbData,
                         0)) {
      ply_close(ply);
      throw std::runtime_error("PLY file \"" + filename +
                               "\" does not contain vertex indices!");
    }
  }
...
void write_ply(const std::string &filename, const MatrixXu &F,
  ...
  if (N.size() > 0 && Nf.size() > 0)
    throw std::runtime_error(
        "Please specify either face or vertex normals but not both!");
...
void load_assimp_common(const std::string &filename, MatrixXu &F, MatrixXf &V,
  ...
  for (unsigned int m = 0; m < scene->mNumMeshes; m++) {
    ...
    for (unsigned int i = 0; i < mesh->mNumVertices; i++) {
      V(0, currentV + i) = mesh->mVertices[i].x;
      V(1, currentV + i) = mesh->mVertices[i].y;
      V(2, currentV + i) = mesh->mVertices[i].z;
